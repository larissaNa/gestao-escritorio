...
const ExportarBeneficios = () => {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        const [listaBeneficios, listaColaboradores] = await Promise.all([
          beneficioService.getAllBeneficios(1000),
  ...
  const opcoesFiltros = useMemo(() => {
    ...
    beneficios.forEach(b => {
      if (b.responsavelUID) {
        const nome = b.responsavelNome || 'Desconhecido';
        responsaveisMap.set(b.responsavelUID, nome);
      }
      
      const data = b.data instanceof Date ? b.data : (b.data as any).toDate?.() || new Date(b.data);
      if (data) {
  ...
  const beneficiosFiltrados = useMemo(() => {
    return beneficios.filter(b => {
      ...
      if (filtroResponsavel !== 'todos' && b.responsavelUID !== filtroResponsavel) return false;
  ...
  }, [beneficios, filtroResponsavel, filtroMes, filtroAno, filtroTipo]);

  const handleExportarPdf = async () => {
    if (!beneficiosFiltrados.length) return;
    ...
    const dadosAgrupados = new Map<string, BeneficioItem[]>();
    beneficiosFiltrados.forEach(b => {
      const uid = b.responsavelUID || 'sem-resp';
      if (!dadosAgrupados.has(uid)) {
        dadosAgrupados.set(uid, []);
      }
      dadosAgrupados.get(uid)?.push(b);
    });
    ...
    gruposOrdenados.forEach(([uid, lista]) => {
      ...
      doc.setFontSize(12);
      doc.text(`${nomeResponsavel} (${setor}) - ${qtd} benefícios`, 50, y + 7);
      y += 25;

      // Tabela de Itens
      autoTable(doc, {
        startY: y,
        head: [['Data', 'Cliente', 'Tipo', 'Subtipo']],
        body: lista.map(item => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Exportar Benefícios" 
        description="Gere relatórios PDF dos benefícios filtrados por período e responsável."
      />
        ...
        <CardContent className="pt-6">
          ...
          <div className="flex justify-end">
            <Button 
              ...
              disabled={loading || beneficiosFiltrados.length === 0}
...
export default ExportarBeneficios;
